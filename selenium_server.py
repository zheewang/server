...
async def fetch_stock_data(stock_codes, url_template, max_concurrency=10):
    ...
    async with async_playwright() as p:
        ...
        tasks = [fetch_one_stock(browser, code, url_template, semaphore) for code in stock_codes]
        results = await asyncio.gather(*tasks, return_exceptions=True)
...
async def fetch_one_stock(browser, code, url_template, semaphore):
    """爬取单个股票数据"""
...
async def main():
    ...
    socket.bind("tcp://*:5555")  # 绑定 5555 端口，等待请求
    logger.info("Selenium Server started on port 5555...")

    print("Async Selenium Server started on port 5555..")
    ...
    while True:
        try:
            message = await socket.recv_json()  # 异步接收请求
            stock_codes = message.get("stocks", [])
            print(f"Received request for stocks: {stock_codes}")
            logger.debug(f"Main thread dispatching request for stocks: {stock_codes}")
            ...
            logger.debug(f"Main thread sent data: {data}")
        except Exception as e:
            logger.error(f"Error: {e}")
            await socket.send_json({})  # 返回空数据，避免客户端卡住
